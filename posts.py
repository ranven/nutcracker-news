...
def get_all_posts(user_id):
    if user_id == 0:
        ...
        (SELECT SUM((v.vote_code)::int)-SUM((not v.vote_code)::int) as votes FROM votes v WHERE v.post_id = p.post_id) 
        FROM posts p
...
def get_post(post_id):
    #todo: add votes
    sql = "SELECT p.title, p.content, p.created_at FROM posts p WHERE p.post_id = :post_id"
